...
def handle_csv(csv_file_name, csv_re_file_name):
	...
	for row in reader:  # 按行处理csv文件
		...
		col_2 = row[2]  # 第三列
		col_3 = row[3]  # 第四列
		title_header = [head_row[1], head_row[2], head_row[3]]
		# 只处理第二列队LST_Day_1km 与 第三列 b1
		lst_data = col_1.lstrip("[").rstrip("]").split(",")  # 去掉[]，并以逗号分隔数据成数组
		lst_data1 = col_2.lstrip("[").rstrip("]").split(",")  # 去掉[]，并以逗号分隔数据成数组
		b1_data = col_3.lstrip("[").rstrip("]").split(",")  # 去掉[]，并以逗号分隔数据成数组
		lst_data_len = len(lst_data)
		lst_data1_len = len(lst_data1)
		...
		for i in range(max(lst_data_len, lst_data1_len, b1_data_len)):
			lst_cell = ""  # 如果lst数据小于b1数据长度，则剩下的lst置为""
			lst1_cell = ""  # 如果lst数据小于b1数据长度，则剩下的lst置为""
			b1_cell = ""  # 如果b1数据小于lst数据长度，则剩下的b1置为""
			if i < lst_data_len:
				lst_cell = lst_data[i].strip()
			if i < lst_data1_len:
				lst1_cell = lst_data1[i].strip()
			if i < b1_data_len:
				b1_cell = b1_data[i].strip()
			row_data = [lst_cell, lst1_cell, b1_cell]
...
if __name__ == "__main__":
	# path = "D:\Document\Google Earth\2"  # 文件目录
	# main(path)
	main(sys.argv[1])
